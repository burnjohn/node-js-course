...
export default class Root extends React.Component {
  ...
  onEnterChatroom(chatroomId, onNoUserSelected, onEnterSuccess) {
    ...
    return this.state.client.join(chatroomId, (err, chatHistory) => {
      if (err)
  ...
  register(name) {
    const onRegisterResponse = user => this.setState(
      {isRegisterInProcess: false, user});
                      ...
                      render={
                        props => (
                          <Home
                            ...
                            onEnterChatroom={
                              (chatroomName, chatroomId) => this.onEnterChatroom(
                                chatroomId,
                    ...
                    />
                    {
                      this.state.chatrooms.map(chatroom => (
                        <Route
                          ...
                          path={`/${chatroom.name}`}
                          render={
                            props => this.renderChatroomOrRedirect(chatroom,
                              props)
                          }
                        />
                      ))
